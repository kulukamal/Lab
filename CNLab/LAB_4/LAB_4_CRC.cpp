...
{
     srand(time(0)); 
	// message and key defined
    string msg = "11110110110010", key = "10011";
	// reminder calculated by mod 2 divison
    string rem = mod2div(msg,key);
	// reminder append to orginal message
    string transMsg = Xor(msg,rem);
	// without any error received message tested 
    cout<<"------------------------- With out Error -----------------------------------------\n";
	...
	// error introduced in the message and then tested
    cout<<"------------------------- With Error ---------------------------------------------\n";
